...
There is no desktop header. `NavigationSidebar` places the brand beside the collapse control and GitHub plus `ThemeSelect` in its footer. The theme selector keeps its palette icon in both expanded and compact navigation, including the mobile dialog. The trigger does not animate its text color, so its label disappears together with the other navigation labels at the container breakpoint; its popup opens upward when needed. The mobile dialog contains the brand and footer too. `MainNavHeader` is only the 44px mobile menu trigger strip. Sidebar and Settings selectors use the same `settings.theme`, `AVAILABLE_THEMES`, and `updateSettings`. No new storage key or navigation preference is persisted.

On desktop, the brand and chevron form one navigation toggle with a subtle shared hover background and a higher-contrast chevron, without a border or shadow. Clicking the logo or name collapses the sidebar instead of navigating home; the Home navigation item remains available. The chevron keeps a fixed, non-shrinking size in both states, and the toggle retains a visible keyboard focus indicator. The footer places the theme selector above GitHub in desktop and mobile navigation.

The home page introduction has a minimum height of its actual scroll container, rather than subtracting a fixed header height from the viewport. It fills the available workspace on desktop and below the mobile menu strip, grows with its content on short screens, and keeps the following information and FAQ sections reachable by scrolling.
...
`PopoverMenu` portals into the nearest dialog or `document.body`. It measures the trigger and available viewport space, repositions on resize/scroll, and can open above its trigger. This avoids clipping inside scrollable tool controls. Dropdown lists are capped at 14.5rem (232 CSS pixels at a 16px root) or the available viewport space, whichever is smaller, with internal scrolling. This fits the six date-format choices without a two-pixel scrollbar. Popups close when their trigger becomes inert, including responsive overflow and collapsed details. Its capture-phase Escape handler consumes the event and returns trigger focus before an enclosing panel can also close. `utils/workspaceKeyboard.ts` prevents background shortcuts from consuming keys handled by tool controls, menus, or open native dialogs.
